...
public class AccountOperationServiceImpl implements AccountOperationService {
    ...
    public AccountHistoryDTO getAccountHistory(String accountId, int page, int size) throws BankAccountNotFoundException {
        ...
        Page<AccountOperation> accountOperations = accountOperationRepository.findByBankAccountId(accountId, (Pageable) PageRequest.of(page, size)); // we want a list of operations done by the account of which its id is passed as parametre
        AccountHistoryDTO accountHistoryDTO=new AccountHistoryDTO(); // creer un objet dto
        List<AccountOperationDTO> accountOperationDTOS = accountOperations.getContent().stream().map(op -> // the list of said account operations
